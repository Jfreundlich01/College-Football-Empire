...
// on success returns an array of critical items for the Peer Supply community
// if there are no critical items in the database, returns 1;
// in case of a search with no results returns 0

const commCritItemsRouter = createRouter<NextApiRequest, NextApiResponse>();

commCritItemsRouter.get(async (req, res) => {
  try {
    ...
    if (searchActive && searchTerm.length > 0) {
      const searchNodes = await neosession.executeRead((tx) => {
        const query = `
          ...
          OPTIONAL MATCH (b:Organization {oid: $oid})-[mc:IN_CRIT_LIST]->(node)
          OPTIONAL MATCH (node)-[:MADE_BY]->(makerOrg)
          RETURN node AS p, COUNT (DISTINCT o) AS prevalence, score, mc, makerOrg
      ...
      // creates count of total organizations
      ...
      const searchResults = sliceSearch.map((record) => {
        ...
        const bold = record.get('mc') ? true : false;
      ...
      const commCritItems = await neosession.executeRead((tx) => {
        const query = `
          ...
          OPTIONAL MATCH (b:Organization {oid: $oid})-[mc:IN_CRIT_LIST]->(p)
          OPTIONAL MATCH (p)-[:MADE_BY]->(makerOrg)
          RETURN p, prevalence, mc, makerOrg
          ORDER BY ${sortStatement}
          SKIP $skip
          LIMIT $limit
      `;
...
export default commCritItemsRouter;
